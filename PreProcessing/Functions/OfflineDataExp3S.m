...
function emgDataTested = OfflineDataExp3S(timeWindow,stepWindow,correctionMode)
...
if userIndex~=dataPacketSize+1
    ...
    if ~(strcmpi(dataPacket(userIndex).name, '.') || strcmpi(dataPacket(userIndex).name, '..'))
        ...
        if correctionMode=="off"
            % This code sets "order" to 1 (No correction)
            
            % order= orientation{energy_index,5};
        else
        ...
        if emgRepetition <= repTraining + repTesting
            ...
            if emgRepetition <=repTraining
                ...
                emgData        = userData.training{emgRepetition,1}.emg(:,WM_X(orientation{energy_index,5})); % ROTATION
                ...
                emgData        = userData.testing{emgRepetition-repTraining,1}.emg(:,WM_X(orientation{energy_index,5})); % ROTATION
                emgData        = emgData(:,sequence_);                                                                   % CORRECTION
