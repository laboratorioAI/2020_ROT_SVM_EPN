...
function [users_to_check,users_good_check,user_names] = Wmoos_check_sensor_placement(name_indices,rotation_Mode,syncro)
    ...
    if ~(strcmpi(folders(i).name, '.') || strcmpi(folders(i).name, '..'))
        ...
        for j = 1:numIndper
            if (strcmpi(folders(i).name, user_names{j,1}))
                ...
                [emg,~]=WM_Extraction_ref_(nameUser,1,0,rotation_Mode,syncro);
                ...
                ref_electrode=pos_;
                %==========================================================
                fprintf('Actual general user is :  %d / %d \n',i-2, numIndper);
